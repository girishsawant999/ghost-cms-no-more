(function ($) {
    ...
    function nf_multiLevel(targetElement = "ul li", mLhasSubmenu = "mL-has-submenu", mLsubmenu = "mL-submenu") {
        ...
        remove_text('dropdown-item', mLchildDetectText);
    ...
    function nf_megamenu(hasMegaMenuClasses = "has-megamenu", col = 3, item_slice = 4, hasMegaMenuDetectText = "[has_megamenu]", submenuUlClasses = "ghost-submenu") {
        ...
        for (let i = 0; i < megaMenuEl.length; i += item_slice) {
            megaMenuEl.slice(i, i + item_slice).wrapAll(`<div class='col-md-${col}'><ul class="list-inline"></ul></div>`);
        }

        for (let i = 0; i < titleText.length; i++) {
            $(`.${submenuUlClasses} > div:eq(${i})`).prepend(`<h6 class="megamenu-title-text text-danger">${titleText[i]}</h6>`);
            $(`.megamenu-title-text:eq(${i})`).text(titleText[i].replaceAll("[title]", ""));
        //    console.log(titleText[i]);
        }
        remove_text(hasMegaMenuClasses, hasMegaMenuDetectText);

        $('ul.dropdown-menu.row').wrap('<ul class="dropdown-menu megamenu"/><div class="row"/>').contents().unwrap();

    }

    function nf_ghost_dropdown(options) {
        ...
        let defultOptions = {
            ...
            hasMegaMenuClasses: "has-megamenu",
        ...
        if (options.mega_menu) {
            nf_megamenu(hasMegaMenuClasses, 3, 6, hasMegaMenuDetectText, submenuUlClasses);
        }


        // Add remove class for newsfeed theme

        $(".mL-has-submenu").addClass("dropdown"); 
        $(".mL-submenu").addClass("submenu dropdown-menu"); 
        ...
        $(".dropdown-menu.megamenu .dropdown-item").removeClass("dropdown-item");

        
        let membershipText = $(".ghost-dropdown-menu li").last().children().text();
        if(membershipText.includes("Membership")){
            $(".ghost-dropdown-menu li").last().children().append("<span>NEW</span>");
        }
    ...
    $(document).ready(function () {
        nf_ghost_dropdown({
            targetElement: ".header-area .navbar-nav li",
            hasChildrenClasses: "dropdown",
            hasChildrenIcon: "<span class='nf-toggle-iocn'>+</span>", 
            hasChildDetectText: "[has_child]",
            submenuUlClasses: "dropdown-menu",
            subitemDetectText: "[subitem]",
            subitemLiClasses: "nav-item dropdown-item",
            multi_level: true,
            mega_menu: true
        });
    });
